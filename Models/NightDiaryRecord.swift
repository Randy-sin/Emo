...
class NightDiaryRecord {
    static let shared = NightDiaryRecord()
    private let defaults = UserDefaults.standard
    private let currentRecordKey = "currentNightDiaryRecord"
    private let recordsKey = "nightDiaryRecords"
    ...
    struct Record: Codable {
        ...
        init(startTime: Date, feeling: Int, events: [String], eventDescription: String, futureExpectation: String) {
            ...
            self.feeling = feeling  // 直接保存用户选择的值（1-5）
            self.events = events
