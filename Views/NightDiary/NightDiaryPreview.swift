...
struct NightDiaryPreview: View {
    @Environment(\.dismiss) private var dismiss
    let record: NightDiaryRecord.Record?
            ...
            HStack(spacing: 0) {
                ...
                VStack(spacing: 4) {
                    Text("创建时间")
                        .font(.system(size: 12))
                        .foregroundColor(.gray)
                    Text(record?.endTime.formatted(date: .omitted, time: .shortened) ?? "--:--")
                        .font(.system(size: 15))
                }
                .frame(maxWidth: .infinity)
...
#Preview {
    NavigationView {
        NightDiaryPreview(record: NightDiaryRecord.Record(
            startTime: Date().addingTimeInterval(-300),
            feeling: 4,
            events: ["运动", "学习"],
            eventDescription: "今天的运动和学习都很顺利",
            futureExpectation: "希望明天也能保持这样的状态"
        ))
    }
} 